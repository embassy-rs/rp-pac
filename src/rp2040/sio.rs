...
impl Sio {
    ...
    pub const fn gpio_out(self, n: usize) -> Gpio {
        assert!(n < 2usize);
        unsafe { Gpio::from_ptr(self.ptr.add(0x10usize + n * 32usize) as _) }
    }
    #[inline(always)]
    pub const fn gpio_oe(self, n: usize) -> Gpio {
        assert!(n < 2usize);
        unsafe { Gpio::from_ptr(self.ptr.add(0x20usize + n * 32usize) as _) }
    }
    #[inline(always)]
    ...
    pub const fn div(self) -> Div {
        unsafe { Div::from_ptr(self.ptr.add(0x60usize) as _) }
    }
    #[inline(always)]
